...
#include <petsc/finclude/petsc.h>
    ...
    ! Dump phi into psi_like1 and RHS into psi_like2
    call VecPlaceArray(psi_like1, phi, ierr)
    call EChk(ierr,__FILE__,__LINE__)
    ...
    call MatMultTranspose(dRdWT, psi_like1, adjointRes, ierr)
    call EChk(ierr,__FILE__,__LINE__)
    ...
    if(myid ==0 .and. printTiming)  then
       ...
       write(*,50) "res init  :", adjResInit
       write(*,50) "res start :", adjResStart
    end if
    ...
    L2abs = adjResStart * relativeTolerance
    ...
    ! adjoints.
    L2Rel = (absoluteTolerance * adjResInit) / adjResStart
   ...
   !  call KSPSolveTranspose(adjointKSP, psi_like1, psi_like2, ierr)
    ...
    ! Now compute the update to psi_like1 (phi)
